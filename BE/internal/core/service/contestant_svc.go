...
func (s *contestantService) CreateProfile(ctx context.Context, userID string, input *domain.Contestant) (*domain.Contestant, error) {
	...
	input.UserID = userID
	input.Status = domain.ContestantStatusPending // Auto-submit for review
...
func (s *contestantService) UpdateProfile(ctx context.Context, userID string, input *domain.Contestant) (*domain.Contestant, error) {
	...
	// User Requirement: Cho phép sửa khi đang chờ duyệt. Chỉ chặn khi đã đỗ hoặc bị loại.
	if current.Status == domain.ContestantStatusApproved || current.Status == domain.ContestantStatusRejected {
...
func (s *contestantService) SubmitProfile(ctx context.Context, userID string) error {
	...
	current.Status = domain.ContestantStatusPending
	current.UpdatedAt = time.Now()
...
func (s *contestantService) DeleteProfile(ctx context.Context, userID string) error {
	...
	current.Status = domain.ContestantStatusDeleted
	current.UpdatedAt = time.Now()
